...
**콘텐츠 타입** (`RidiContentType`):

| 코드 | 설명 | URL slug 예시 |
|------|------|-------------|
| `ebook` | e북 (기본값) | `bestsellers/fantasy` |
| `webnovel` | 웹소설 | `bestsellers/fantasy_serial` |

웹소설 장르별 slug 매핑:

| 장르 | e북 | 웹소설 |
|------|-----|--------|
| 판타지 | `fantasy` | `fantasy_serial` |
| 로판 | `romance_fantasy` | `romance_fantasy_serial` |
| 로맨스 | `romance` | `romance_serial` |
| BL | `bl-novel` | `bl-webnovel` |
...
**DB 저장**: `raw_list_items`에 upsert (platform + external_id + content_type 기준)
...
| 콘텐츠 타입 | breadcrumb / `#ISLANDS__Header` / meta keywords에서 "웹소설"/"e북" 감지 |

키워드에서 `^작품$`, `^완결$`, `^기다리면 무료$` 등 노이즈를 필터링한다.

**content_type 감지**: 상세 페이지 HTML에서 "웹소설" → `webnovel`, "e북" → `ebook`을 breadcrumb → Header → meta keywords 순으로 탐색한다.

**DB 저장**: `raw_work_parse_results`에 insert (content_type 포함)
...
#### migrations/004_content_type.ts

`raw_list_items`와 `raw_work_parse_results`에 `content_type` 컬럼 추가:
- `raw_list_items.content_type` (varchar, NOT NULL, default `'ebook'`) — e북/웹소설 구분
- `raw_work_parse_results.content_type` (varchar, nullable) — 상세 페이지에서 감지된 콘텐츠 타입

### Schemas (Zod)

#### raw.schema.ts — Raw Zone 검증 스키마

`Platform`, `ListType`, `ContentType`, `RawListItem`, `ParsedWorkData` 등 원본 데이터 구조 정의.
- `ContentType`: `ebook | webnovel`
...
| `-t, --type` | 콘텐츠 타입 (`ebook`, `webnovel`, `all`) | `ebook` |
| `-p, --page` | 시작 페이지 | `1` |
| `--pages` | 크롤링할 페이지 수 | `1` |
| `-l, --limit` | 총 아이템 수 제한 | - |

```bash
# 판타지 e북 스테디셀러 1페이지 (기본)
yarn crawl:list -g fantasy

# 판타지 웹소설 스테디셀러
yarn crawl:list -g fantasy -t webnovel

# 로맨스 웹소설 주간 베스트셀러 3페이지
yarn crawl:list -g romance -t webnovel -o weekly --pages 3

# 전 장르 e북 + 웹소설 모두 60개씩
yarn crawl:list -g all -t all -l 60
```
